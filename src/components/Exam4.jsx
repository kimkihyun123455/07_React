...
const Exam4 = () => {
  // 상태
  // 자식 컴포넌트의 상태 id, pw를 부모로 끌어올려 작성성
  const [pw, setPw] = useState("");
  const [id, setId] = useState("");

  // 자식 컴포넌트의 상태 변경 함수도 부모로 끌어올려 작성성
  // ID 상태를 업데이트 해주는 함수
  const onChangeId = (e) => {
  ...
  return (
    <div>
      ...
      <div>
        {/* 자식이 가진 id, pw라는 상태값을 부모 컴포넌트가 알 방법이 없었다
        -> 부모 컴포넌트로 자식의 상태, 함수를 끌어올려 사용했다 */}
...
const Id = (props) => {
  return (
    <div>
      <label>ID : </label>
      {/* props에 담긴 onChangeId라는 key에 매핑된 value값을 사용 */}
