...
class Exam1 extends Component {
  // React.Component 클래스 상속 받아 클래스형 컴포넌트 정의

  // 생성자 정의.
  // props : 부모 컴포넌트로부터 전달 받은 데이터터
  constructor(props) {
    super(); // 부모 클래스의 생성자를 호출

    // this.state : 클래스형 컴포넌트에서 컴포넌트의 상태 객체를 의미함
    this.state = { count: 0 }; // state(상태) 중 count 상태값을 초기화.
  ...
  render() {
    return (
      <div>
        ...
        <h2>부모로부터 전달 받은 값 : {this.props.aaa}</h2>
      </div>
    );
  }
}

// 다른 파일에서 해당 파일의 기본으로 지정된 컴포넌트를 사용할 수 있도록
// 내보냄냄
